  ...
  return (
    <Layout>
      ...
      <div className="card overflow-hidden flex-grow overflow-y-auto">
        <div className="grid grid-cols-[1fr_1fr_1fr_1fr] px-5 py-3 border-b border-divider text-[11.5px] font-bold text-text-muted uppercase tracking-wide">
          <div>Nombre</div><div>Teléfono</div><div>Email</div><div>Dirección</div>
        </div>
        {clinicas.map((c) => (
          <div key={c.id} className="grid grid-cols-[1fr_1fr_1fr_1fr] px-5 py-3.5 border-b border-[#F0F2F1] text-[13.5px] items-center">
            <div className="font-semibold">{c.nombre}</div>
            <div className="text-text-secondary">{c.telefono || '—'}</div>
            <div className="text-text-secondary">{c.email || '—'}</div>
