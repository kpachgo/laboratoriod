    ...
    <Layout>
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 bg-primary text-white rounded-input px-4 h-10 text-[13px] font-semibold"
        >
          <PlusIcon />
          Nuevo usuario
        </button>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card p-5 grid grid-cols-2 gap-3.5">
          ...
          {form.rol === 'clinica' && (
            <select className="field-input col-span-2" required value={form.clinicaId} onChange={(e) => setForm({ ...form, clinicaId: e.target.value })}>
          ...
          <button disabled={saving} className="col-span-2 h-10 rounded-input bg-primary text-white text-sm font-semibold">
      ...
      <div className="card overflow-hidden flex-grow overflow-y-auto">
        <div className="grid grid-cols-[1fr_1fr_1fr_80px] px-5 py-3 border-b border-divider text-[11.5px] font-bold text-text-muted uppercase tracking-wide">
          <div>Nombre</div><div>Usuario</div><div>Rol</div><div>Activo</div>
        </div>
        {usuarios.map((u) => (
          <div key={u.id} className="grid grid-cols-[1fr_1fr_1fr_80px] px-5 py-3.5 border-b border-[#F0F2F1] text-[13.5px] items-center">
